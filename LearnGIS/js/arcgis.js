...
require(["esri/map", "esri/dijit/Scalebar", "esri/dijit/Geocoder", "esri/request",
  "http://esri.github.io/bootstrap-map-js/src/js/bootstrapmap.js", "dojo/domReady!"],
  function(Map, Scalebar, Geocoder, esriRequest, BootstrapMap) {
    var mapLocations = [
    ...
    var map = new Map("mapDiv", { basemap:firstLoc[0], center:firstLoc[1], zoom:firstLoc[2],
      isKeyboardNavigation:true, isClickRecenter:true, isPanArrows:true, isShiftDoubleClickZoom:true});
    map.on("update-end", function() { if (playing) nextMap(false); });
    map.on("extent-change", function() { if(hopscotch.getCurrStepNum()==1) hopscotch.nextStep(); });
    ...
    function pauseTour() {
      if(hopscotch.getCurrStepNum()==4) hopscotch.nextStep();
    ...
    $(document).ready(function() {
      ...
      var tour = esriRequest({ url: "index.tour.json", handleAs: "json" });
